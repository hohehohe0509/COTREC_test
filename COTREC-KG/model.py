...
class COTREC(Module):
    ...
    def sess_final_get(self, hidden, mask, target_emb, att_hidden, relation_emb):  #Dual_att[b,d], Dual_g[b,d]
        # # ht为local_embedding
        ...
        #SIC
        neighbor_sess = self.FindNeighbor(relation_emb)
        sess_final = neighbor_sess #[b,d]
    ...
    def train_loss(self, data, sessionID, session_item, session_len, D, A, reversed_sess_item, mask, epoch, tar, diff_mask, kg, g):
        ...
        #這裡每個seesion的item順序是倒過來的
        hy_item_embeddings = self.generate_item_pos(item_embeddings_i, session_item, session_len, reversed_sess_item, mask)
        sess_emb_kg = self.generate_sess_emb(item_embeddings_kg, session_item, session_len,reversed_sess_item, mask)

        zeros = torch.cuda.FloatTensor(hy_item_embeddings.shape[0], 1, self.dim).fill_(0)  # [b,1,2d]
        session_target = torch.cat([hy_item_embeddings, zeros], 1)  # [b,s+1,2d]
        ...
        relation_emb = self.RelationGraph(self.embedding, session_item, A, D, session_len, target_emb)
        sess_emb_i = self.sess_final_get(hy_item_embeddings, mask, target_emb, x_n, relation_emb)
